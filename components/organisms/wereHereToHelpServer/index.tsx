...
const WereHereToHelpServer = async ({ dict }: { dict: DictProps }) => {
  try {
    const userHash = await cookieUtils.get(COOKIE_KEYS.USER_INFO);
    const userInfo: UserProps = JSON.parse(decrypt(userHash as string));

    return <WereHereToHelp dict={dict} userInfo={userInfo} />;
  } catch (err) {
    console.log("errorNavbarHome server: ", err);
    return <WereHereToHelp dict={dict} />;
  }
};

export default WereHereToHelpServer;
