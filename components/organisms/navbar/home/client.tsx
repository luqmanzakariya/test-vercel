...
import BelibizWhiteIcon from "@/components/atoms/icons/belibiz-white";
import BelibizIcon from "@/components/atoms/icons/belibiz";

const NavbarHomeClient = ({ dict, userInfo }: { dict: DictProps; userInfo?: UserProps }) => {
    ...
    <>
      <div className="px-4 lg:px-[0px] pt-[4px] lg:pt-7 lg:pl-[61px] lg:pr-[47px] block lg:flex lg:items-center lg:justify-between mb-6">
        ...
        <div className="flex justify-between items-center mt-2 lg:mt-0 mb-6 lg:mb-0">
          <Link href="/">
            <BelibizWhiteIcon className="w-[135px] lg:w-[270px] h-[25px] lg:h-[50px] lg:mb-[0] hidden lg:flex" />
            <BelibizIcon className="w-[135px] lg:w-[270px] h-[25px] lg:h-[50px] lg:mb-[0] flex lg:hidden" />
          </Link>
          {/* Mobile Version */}
            ...
            {userInfo?.id ? (
              ...
              <div className="lg:min-w-[18rem] flex justify-end">
                ...
                >
                  <Button
                    variant="secondary"
                    style="w-[102px] lg:w-[130px] h-[41px] lg:h-[55px] mt-2 lg:mt-0 bg-white"
                  >
                    <div className="flex items-center gap-[12px]">
                      {dict.common.login}
                      <LockIcon />
        ...
        <div className="flex items-center justify-around lg:justify-between w-full lg:w-[523px] gap-2 lg:gap-[0px] mx-auto lg:mx-0">
          <Link href="/" className="nabar-items text-[18px] lg:text-[24px]">
            {dict.navbar.home}
          </Link>
          <Link href={APP_ROUTE.LISTING} className="nabar-items text-[18px] lg:text-[24px]">
          ...
          {userInfo?.id ? (
            <>
              {userInfo?.type !== "ADMIN" ? (
                ...
                <div className="lg:min-w-[10rem] flex justify-end"></div>
              )}
              <Popover>
                <PopoverButton className="outline-none">
            ...
            <div className="lg:min-w-[18rem] flex justify-end">
              <Link
                ...
                replace={true}
              >
                <Button variant="secondary" style="w-[130px] h-[55px] mt-2 lg:mt-0 bg-white">
                  <div className="flex items-center gap-[12px]">
                    {dict.common.login}
                    <LockIcon />
                  </div>
