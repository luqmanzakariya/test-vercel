  ...
  return (
    <div className="flex flex-col items-center rounded">
      <div className="bg-white rounded-[25px] overflow-hidden shadow-lg w-full">
        <div className="p-5">
          {data?.business_listing?.media?.photos &&
          data?.business_listing?.media?.photos.length > 1 ? (
            <Image
              className="w-full h-[300px] object-cover rounded-xl mb-5"
              src={data?.business_listing?.media?.photos[0]}
              alt="Property Image"
              width={500}
              height={500}
            />
          ...
          <div className="text-2xl truncate">{data?.business_listing?.title}</div>
          <div className="text-gray-400 mb-5 truncate">{data?.business_listing?.description}</div>
          <div className="mb-7 truncate">{data?.business_listing?.reason}</div>
          ...
          <div className="flex justify-items-stretch mb-2">
            <div className="w-full text-2xl flex items-center">
              {data?.business_listing?.currency}{" "}
              {thousandSeparator(data?.business_listing?.price || 0)}
            </div>
            <div className="flex items-center justify-end">
              <button
                className="bg-red-800 text-white rounded-full p-3"
                onClick={() => goToDetailPage(data?.business_listing?.id)}
              >
                <ArrowSlightUp />
              </button>
            </div>
...
export default CardWishlistProperty;
